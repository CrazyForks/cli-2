...
export function readSql(values, cwd = process.cwd()) {
  if (values.sql && values.file) throw new CliError("pass only one of --sql or --file");
  if (values.sql) return values.sql;
  if (values.file) {
...
export function readMigrationFiles(dir = "migrations") {
  ...
  try {
    ...
    throw new CliError(`cannot read migrations dir ${dir}: ${err.message}`);
  }
  return entries
